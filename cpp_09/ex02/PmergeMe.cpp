...
std::list<int>& PmergeMe::sortList(std::list<int>& lst) {
	...
	while (it != lst.end()) {
		...
		if (it != lst.end()) {
			int second = *it++;
			pairs.push_back(std::make_pair(std::min(first, second), 
	...
	for (pairIt = pairs.begin(); pairIt != pairs.end(); ++pairIt) {
		...
		while (insertPos != sortedPairs.end() && 
			   pairIt->first >= insertPos->first) {
			++insertPos;
		}
		sortedPairs.insert(insertPos, *pairIt);
	}

	std::list<int> mainChain;
	std::list<int> pendChain;

	for (pairIt = sortedPairs.begin(); pairIt != sortedPairs.end(); ++pairIt) {
		mainChain.push_back(pairIt->first);
		pendChain.push_back(pairIt->second);
	}

	std::list<int>::iterator pendIt;
	for (pendIt = pendChain.begin(); pendIt != pendChain.end(); ++pendIt) {
	...
	if (!remaining.empty()) {
		std::list<int>::iterator insertPos = mainChain.begin();
		while (insertPos != mainChain.end() && 
			   *insertPos <= remaining.front()) {
			++insertPos;
		}
...
void PmergeMe::sort(int argc, char** argv) {
	...
	printFirstElementsLst(_lst, 4);
	std::cout << "After:  "; //TODO
	printFirstElementsDeq(_deq, 4); //TODO
